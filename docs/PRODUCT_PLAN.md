...
每次从某轨开始实际输出音频时创建一个 `PlaySession`：
...
立即进入 P0，但先完成以下仓库基础工作：

1. 检查并记录本机 C++、CMake、Windows SDK 和构建工具。
2. 建立 CMake 项目骨架和模块目录。
3. 编写第一份 ADR：整盘绝对采样时间轴。
4. 实现不依赖 UI 的 LBA、MSF、扇区和采样帧转换库及测试。
5. 实现光驱枚举与 TOC 读取探针。
6. 实现合成 PCM/CDDA 源和跨轨无缝测试夹具。
7. 最后接入真实光驱连续读取和 WASAPI 输出。
